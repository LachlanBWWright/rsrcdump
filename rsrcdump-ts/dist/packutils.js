...
export class Unpacker {
    ...
    unpack(fmt) {
        ...
        while (i < fmt.length) {
            ...
            if (type === 'x') {
                ...
                // Byte string
                values.push(this.data.slice(this.offset + pos, this.offset + pos + count));
                pos += count;
            }
            else if (type === 'B') {
    ...
    unpackPstr(encoding = 'macroman', _errors = 'replace') {
        const [length] = this.unpack('>B');
        const bytes = this.read(length);
        // Simplified encoding handling
        return Buffer.from(bytes).toString(encoding === 'macroman' ? 'latin1' : encoding);
    }
    eof() {
    ...
    pack(fmt, ...values) {
        ...
        while (i < fmt.length) {
            ...
            if (type === 'x') {
                ...
                for (let j = 0; j < count; j++) {
                    bytes.push(values[valueIdx++] & 0xFF);
                }
            }
            else if (type === 'b') {
                for (let j = 0; j < count; j++) {
...
export class WritePlaceholder {
    ...
    calcSize(fmt) {
        ...
        while (i < fmt.length) {
            ...
            const type = fmt[i];
            if (type === 'x' || type === 'B' || type === 'b')
                size += count;
            else if (type === 'H' || type === 'h')
                size += 2 * count;
            else if (type === 'L' || type === 'l' || type === 'i' || type === 'f')
                size += 4 * count;
            else if (type === 'Q' || type === 'q' || type === 'd')
                size += 8 * count;
            else if (type === 's')
                size += count;
            i++;
 ...
 */
export function packPstr(text, padding, encoding = 'macroman') {
    const encoded = Buffer.from(text, encoding === 'macroman' ? 'latin1' : encoding);
...
//# sourceMappingURL=packutils.js.map