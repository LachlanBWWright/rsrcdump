...
export function getGlobalEncoding() {
    return GLOBAL_ENCODING;
}
export function setGlobalEncoding(encoding) {
    GLOBAL_ENCODING = encoding;
}
/**
 * Sanitizes a resource type name for use in filenames/URLs
 */
export function sanitizeTypeName(restype) {
    if (restype.length !== 4) {
 ...
 */
export function parseTypeName(saneName) {
 ...
 */
export function decode(bytes, _errors = 'replace') {
    ...
    return Buffer.from(bytes).toString(GLOBAL_ENCODING);
}
/**
 * Encodes string to bytes using the global encoding
 */
export function encode(text, _errors = 'replace') {
    if (GLOBAL_ENCODING === 'macroman') {
        return new Uint8Array(Buffer.from(text, 'latin1'));
    }
    return new Uint8Array(Buffer.from(text, GLOBAL_ENCODING));
}
//# sourceMappingURL=textio.js.map