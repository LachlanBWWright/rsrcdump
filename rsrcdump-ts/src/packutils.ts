...
export class Unpacker {
  ...
  unpackPstr(encoding: string = 'macroman', _errors: 'replace' | 'ignore' = 'replace'): string {
    const [length] = this.unpack('>B');
    const bytes = this.read(length as number);
    // Simplified encoding handling
    return Buffer.from(bytes).toString(encoding === 'macroman' ? 'latin1' : encoding as BufferEncoding);
  }
...
export class WritePlaceholder {
  ...
  private calcSize(fmt: string): number {
    ...
    while (i < fmt.length) {
      ...
      else if (type === 'L' || type === 'l' || type === 'i' || type === 'f') size += 4 * count;
 ...
 */
export function packPstr(text: string, padding: number, encoding: string = 'macroman'): Uint8Array {
  const encoded = Buffer.from(text, encoding === 'macroman' ? 'latin1' : encoding as BufferEncoding);
